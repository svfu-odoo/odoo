...
class AccountMoveLine(models.Model):
    ...
    @api.depends('currency_id', 'company_id', 'move_id.currency_rate', 'move_id.date')
    ...
    def _compute_price_subtotal_before_discount(self):
        for line in self:
            ...
            if line.display_type != 'product':
                line.price_subtotal_before_discount = False

            if line.tax_ids:
                taxes_res_before_discount = line.tax_ids.with_context(round=False, round_base=False).compute_all(
                    line.price_unit,
                    quantity=line.quantity,
                    currency=line.currency_id,
                    product=line.product_id,
