...
class AccountMove(models.Model):
    ...
    def copy_data(self, default=None):
        ...
        for move, vals in zip(self, vals_list):
            ...
            # TODO: maybe do not consider journal if inactive? since  it would be deleted from the vals?
            user_fiscal_lock_date = move.company_id._get_user_fiscal_lock_date(journal=move.journal_id)
            ...
            for chain_moves in journal_moves.grouped('sequence_prefix').values():
                ...
                domain = self.env['account.move']._get_move_hash_domain([
                    *common_domain,
                    ...
                    ('inalterable_hash', '=', False),
                    ('date', '>', last_move_in_chain.company_id._get_user_fiscal_lock_date(journal=journal)),
                ], force_hash=True)
    ...
    def _unlink_or_reverse(self):
        ...
        for move in self:
            lock_date = self.company_id._get_user_fiscal_lock_date(journal=move.journal_id)
